...
public class FeiJi_Play2 extends CCColorLayer implements SensorEventListener{
    ...
    private Sensor s;
	protected FeiJi_Play2(ccColor4B color,Context context) {
		super(color);
	...
	public void setPlayPostion()
	{
		_FeiJi_Play.setPosition(_Touch_Location.x,_Touch_Location.y);
		_Touch_Location.x+=addx;
		_Touch_Location.y+=addy;
        if (_Touch_Location.x<50) {
			addx=Math.abs(addx*0.5f);
		}
        
        if (_Touch_Location.x>getWidth()-50) {
			addx=-Math.abs(addx*0.5f);
		}
        
        if (_Touch_Location.y<60) {
			addy=Math.abs(addy*0.5f);
		}
        
        if (_Touch_Location.y>getHeight()-60) {
			addy=-Math.abs(addy*0.5f);
		}
	}
	...
	public void onSensorChanged(SensorEvent event) {
		setAddx(-event.values[0]*2);
		setAddy(-event.values[1]*2);
	}
	...
	public boolean ccTouchesMoved(MotionEvent event) {
		...
		if (_Can_Move) {
			_Touch_Location = CCDirector.sharedDirector().convertToGL(
			  ...
			  if ((_Touch_Location.x-Location.x)!=0 && (_Touch_Location.y-Location.y)!=0) {
				  _FeiJi_Play.setPosition(_Touch_Location.x, _Touch_Location.y);
			   }
