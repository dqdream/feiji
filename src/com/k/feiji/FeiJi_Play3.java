...
public class FeiJi_Play3 extends CCColorLayer {
	...
	protected FeiJi_Play3(ccColor4B color) {
		super(color);
	...
	private void Init() {
		...
		_FeiJi_Pause.setPosition(CGPoint.make(
				_FeiJi_Pause.getContentSize().width / 2 + 5, _WinSize.height
						- _FeiJi_Pause.getContentSize().height / 2 - 5));
	...
	public boolean ccTouchesMoved(MotionEvent event) {
		if (event.getX() >= 30 && event.getY() >= 40
				&& event.getX() <= (getWidth() - 30)
				&& event.getY() <= (getHeight() - 40)) {
			if (_Can_Move) {
				_Touch_Location = CCDirector.sharedDirector().convertToGL(
						CGPoint.ccp(event.getX(), event.getY()));
				if ((_Touch_Location.x - Location.x) != 0
						&& (_Touch_Location.y - Location.y) != 0) {
					_FeiJi_Play.setPosition(_Touch_Location.x,
							_Touch_Location.y);
				}
	...
	private void AddScore() {
		...
		_ScoreLabel.setPosition(CGPoint.ccp(_FeiJi_Pause.getContentSize().width
				+ 12 + _ScoreLabel.getTexture().getWidth() / 2, _WinSize.height
				- _FeiJi_Pause.getContentSize().height / 2 - 5));
